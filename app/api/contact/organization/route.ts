...
export async function POST(req: NextRequest) {
  try {
    ...
    const { organizationName, contactPerson, email, partnershipDetails } = body;

    // Validate required fields
    if (!organizationName || !contactPerson || !email || !partnershipDetails) {
    ...
    const emailContent = {
      ...
      subject: `Partnership Request from ${organizationName}`,
      html: `
        <!DOCTYPE html>
        <html>
          <head>
          ...
          <body>
            <div class="container">
              <div class="header">
                <h2 style="margin: 0;">Partnership Request</h2>
                <p style="margin: 10px 0 0 0; opacity: 0.9;">The Collective Counsel Partnership Program</p>
              </div>
              <div class="content">
                <div class="highlight">
                  <h3 style="margin: 0 0 10px 0; color: #1a472a;">New Partnership Opportunity</h3>
                  <p style="margin: 0;">An organization is interested in partnering with TCC!</p>
                </div>
                
                <div class="field">
                  <div class="label">Organization Name:</div>
                  <div class="value"><strong>${organizationName}</strong></div>
                </div>
                ...
                <div class="field">
                  <div class="label">Partnership Details:</div>
                  <div class="message-box">${partnershipDetails.replace(/\n/g, '<br>')}</div>
                </div>
                
                <div class="footer">
                  <p>This partnership request was submitted through the TCC website contact form.</p>
    ...
    if (!emailServiceReady) {
      console.error('Email service unavailable - partnership request cannot be processed');
      return NextResponse.json(
    ...
    try {
      ...
      await transporter.sendMail({
        ...
        replyTo: email, // Allow direct reply to organization
      });

      // Log successful submission
      console.log('✅ Partnership request email sent successfully:', {
        organizationName,
        contactPerson,
        email,
      ...
      return NextResponse.json({
        ...
        data: { organizationName, contactPerson, email },
      });
